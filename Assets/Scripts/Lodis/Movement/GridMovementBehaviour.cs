...
namespace Lodis.Movement
{
    ...
    public class GridMovementBehaviour : SimulationBehaviour
    {
        ...
        private FixedPoints.MoveAction _moveLerp;
        ...
        private void LerpPosition(FVector3 newPosition)
        {
            ...
            if (_moveLerp == null)
            {
                _moveLerp = (FixedPoints.MoveAction)FixedLerp.DoMove(FixedTransform, newPosition, TravelTime);
            }
            else
            {
                _moveLerp.Rewind();
                _moveLerp.ChangeValues(FixedTransform.WorldPosition, newPosition);
            }

            _moveLerp.onComplete += ResetMovementValues;

            //UnityEngine.Debug.Log($"Lerping to {TargetPanel.Position}");
        }
        ...
        {
            if (!IsMoving || _previousPanel == null || _targetPanel == null)
                return;


            Fixed32 currentDistance = FVector3.Distance((FVector3)_previousPanel.transform.position, (FVector3)transform.position);
        ...
        public override void Tick(Fixed32 dt)
        {
            ...
            SetIsMoving(dist >= _targetTolerance && (_moveLerp != null && _moveLerp.IsPlaying()));
